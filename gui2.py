...
class dynoGUI:
    def __init__(self, d):
        ...
        welcome_message = Text(self.app, text="Welcome to the Dyno GUI",
           size=30, color="blue", grid=[1,0], width="fill")

        dinopicture = Picture(self.app, image="dinosaur1.gif", grid=[0,0])
        dinopicture2 = Picture(self.app, image="dinosaur1.gif", grid=[2,0])
    ...
    def displayValues(self):
        one = Text(self.app, text="diammeter: " + str(mydyno.wheelDiameter), grid=[1,7], align="left")
        two = Text(self.app, text="horsepower: " + str(mydyno.horsepower), grid=[1,8], align="left")
        three = Text(self.app, text="rpm: " + str(mydyno.rpm), grid=[1,9], align="left")
        four = Text(self.app, text="speed: " + str(mydyno.speed), grid=[1,10], align="left")
        
        #calculates the twerk
        #mydyno.calculate()    
        #five = Text(self.app, text="torque: " + str(mydyno.torque), grid=[1,11], align="left")
    ...
    def setup_gui(self):
        ...
        Diameter
        """
        d_description = Text(self.app, text="Diameter (mm): ", grid=[1,1], align="left")
        input_diameter = TextBox(self.app, grid=[1,1], width=10)
        update_diameter = PushButton(self.app, command=self.updateD ,text="Update Diameter",
            grid=[1,2])
        update_diameter.update_command(command=self.updateD, args=[input_diameter])
        ...
        update_horsepower.update_command(command=self.updateH, args=[input_horsepower])
        
        show_info = PushButton(self.app,command=self.displayValues, text= "Calculate Results", grid=[1,6])
